...
def evaluate(gold, predicted):
    ...
    for question_id, references in gold.items():
        ...
        if "unanswerable" in answer.lower():
            answer = "unanswerable"
        elif answer.lower().startswith("yes") or answer.lower().startswith(" yes"):
            answer = "Yes"
        elif answer.lower().startswith("no") or answer.lower().startswith(" no"):
            answer = "No"
        ...
        if "unanswerable" not in answer.lower():
            if answer_type == "none":
                noans_dict["no"].append(0.0)
            else :
                noans_dict["has"].append(0.0)
        else:
            if answer_type == "none":
                noans_dict["no"].append(1.0)
            else :
                noans_dict["has"].append(1.0)
    ...
    return {
        ...
        "Noans": {key: (sum(value), len(noans_dict[key]),sum(value)/len(noans_dict[key]))for key, value in noans_dict.items()},
